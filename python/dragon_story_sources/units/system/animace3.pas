{Pavel: chyba: nekdy to nereaguje na klik mysi.
        duvod: testovalo se to na miste, kam se obcas vubec
               nemuselo skocit
        opraveno: premisteno hned na zacatek obsluhy mysiho
                  preruseni}

{Bob: premisteno AObjsAddressTab do interface a taky paobjtab
      a tanimcommand
      řádek 1978
      1316: vždyť ty v backtovirginaobj dáváš do aobjsordertab něco na
            úplně nesmyslnou pozici!!!!!
            nebudu to používat..., ale stejně jsem to spravil...
      CompareMouseDelay: to chceš říct, že posunu-li myš a bude to míň než
                         2x zobrazeno, že se nezmění její tvar?
?nevyvolávás SetTextDimensions nějak často a přitom on je pomalej!?!
předělej prosím Tě třídění na nebublinkové; asi je v něm někde chyba,
neboť se ta tabulka časem zmrší!!!!!
ano, tam byla ta chyba!!!!! za to můžeš TY!!!!!
a analogickou chybu jsem udělal kdysi i já při dealokaci bez počkání
na UnVisible...
přidáno visibleaobj1Bob pro vlastní použití...
nechtělo by to vyřadit put... z tohoto unitu?????
Luky, dodělej ten putmaskimagezoompart, jak jsem Ti řekl.
UNDO

pridano settextdimensions do interface (ja to vyvolavam)
1512:(Bob)panacka nebere jako objekt pod mysi; nevim, zda je to dobre...}
...
{PP: ctete vsechno na zacatku, dulezite veci jsou prakticky vsude!!!!}



{PP: Navrhuji prepsat animacni mysku tak, aby podporovala
 primo nekolik kurzoru. Dosavadni prepinani pres MouseOn,
 MouseOff je ponekud neohrabane (prestoze nyni jiz vse funguje
 OK). MouseOn by se mela volat jen jednou, na zacatku programu,
 MouseOff na konci. Vypinani a zapinani mysky v mistnostech
 by se resilo pomoci MouseSwitchOn/Off (na to ty procedury
 taky jsou!)
 Pridaly by se procedury na prepinani kurzoru a na jakesi
 zaregistrovani vice kurzoru. Vymyslete si, co by vam vyhovo-
 valo a ja to napisu.
 Jako navrh prikladam definici, jak jsem ji zkousel zacit
 psat ja:

 procedure AnimMouseInitImage(Num: byte; Image: pointer);
 Myska bude mit interni pole pointeru ukazujicich na ruzne obrazky
 mysi. Napr. [1]..chuze, [2]..chuze aktivni, atd...
 Touto procedurou by se proste jenom priradili pointery, volala
 by se pred MouseOn. MouseOn by vybrala ze vsech obrazku nejvetsi
 a podle neho by vyhradila misto pro uchovani pozadi.

 procedure AnimMouseSetImage(Num: byte);
 Volala by se, kdyz bychom chteli menit obrazek mysi.

 Jak vidite, pouziti by zustalo v podstate uplne stejne, jenom byste
 vsude vyskrtali dvojice MouseOff&MouseOn a nahradili byste je jedinym
 volanim a na zacatku programu byste provedli zaregistrovani ruznych
 obrazku.
 Je to takove pekne a elegantni!
}

{PP: Zmenil jsem usporadani definic, procedur ve zdrojaku.
 Pro toho, kdo se v tom dosud nevyznal, je to prehlednejsi,
 pro toho, kdo se v tom orientoval po pameti, to bude mozna
 o neco obtiznejsi.
 -uplne na zacatku jsem nechal veci tykajici se ladeni a
  konstanty, ktere se kompiluji
 -dale jsem interface rozdelil na tri casti- _ruzne_, _myska_
  a _animace_ (prave takhle jdou po sobe ve zdrojaku)
  V implementaci tytez casti naleznete pod uplne stejnymi jmeny
 -vsechny definice typu, promennych... jsem uvedl v tech oddilech,
  kam logicky patri- jak v interface, tak v implementaci
 !veci tykajici se porovnavani mysky s objekty jsou v _animace_
}

{PP: Patch chyby (Animace pod mysi zustavala obcas nesmazana)
 Chyba byla zpusobena volanim uzivatelske procedury
 z CompareMous... Tato uzivatelska procedura vypinala mysku
 pomoci MouseOff, ktere vola MouseSwitchOff. MouseSwitchOff
 placlo viditelne pozadi na viditelnou, obdobne neviditelne.
 Problem byl v tom, ze v jistych okamzicich myska na obrazovce
 vubec nebyla a sebrane pozadi nebylo aktualni, i kdyz MouseSwitchOff
 predpokladalo opak a placalo to tam, at tam kurzor mysi byl
 vytisteny, nebo nebyl, atd.
 Chyba odstranena nasledovne: zaveden interni indikator pro
 obe stranky, jestli je kurzor mysi vytisteny na one strance.
 Pokud ma v rutinkach kdekoli dojit k placnuti pozadi pres kurzor,
 otestuje se nejdriv, jestli tam kurzor skutecne je a jen v takovem
 pripade se pozadi placne.
}

{PP: CompareMouseWithObjects: problemy se vyskytnou, chceme-li porovnavat
 s obrazkem, ktery ma jine rozmery, nez originalni a/nebo ktery je
 zrcadleny!
 V takovem pripade to nebude fungovat, porovnava to totiz vzdy
 jenom s originalem!
 Pripominka, ze to vpravo neukazuje spravne, se zamita!!! Zkousel jsem
 to TADY a funguje to jak ma!

 Take navrhuji z CompareMouseWithObjects vyfiltrovat draka, ktery bude
 mit od zacatku do konce cislo nula. Jednoduse ho nebudeme porovnavat.
 Je to dobry navrh? -uhhh, mozna, ze ne!
}

{zmeny: inicializace 1->ffff, v rez. casti se vyuziva, ze driver nam rekne
        pozici mysi (nevola se fce), pamatuje si naposled zmacklou klavesu
        a misto MouseKey se bude volat AnimMouseKey; pridano animinitmouse}

{Co by bylo mozne zmenit:
 -pocet predavanych parametru u Add...AObj
 -nazvy proc. zacinajici na New... zmenit na Set...
 }
...
procedure AddAObj(var ObjNum: byte; DefTypeOfObj: TAnimObject;
   DefPriority, DefLastSprite, DefDisableErasing: byte;
...
procedure AddImageAObj(var ObjNum: byte; DefPriority, DefLastSprite,
...
procedure AddTextAObj(var ObjNum: byte; DefPriority: byte;
   StartX, StartY: integer; _Text: PString; _Font: PFont; Color: byte);
...
procedure AddSpriteToObj(ObjNum, SpriteNum: byte; SpriteAddress: pointer);
{prida adresu urceneho sprajtu do tabulky jednoho objektu cislo ObjNum}
procedure AddSpriteToLastObj(SpriteNum: byte; SpriteAddress: pointer);
{prida adresu urceneho sprajtu do tabulky posledniho objektu }

procedure ReleaseLastAObj;
...
procedure ChangePriorityAObj(ObjNum, DefPriority: byte);
...
procedure NewSpriteAObj(ObjNum, DefNewSprite: byte);
{prechod na novy sprajt, zpravidla jiny nez dosavadni, jinak by tato
 procedura nemela ani smysl. Prakticky pouzitelna jenom s objektem
 typu Image, objekt typu Text stejne nemuze mit vic sprajtu}
...
procedure NewZoomAObj(ObjNum: byte; DefZoomedWidth, DefZoomedHeigth: integer);

procedure SetDisableErasingAObj(ObjNum, Setting: byte);
...
type
  ...
  TAObjTab= record
     ...
     LastSprite: byte;  {cislo posledniho sprajtu, ktery objekt obsahuje.
                         Jestlize se objekt sklada prave z jednoho sprajtu,
                         je zde 0 }
     ...
     Mirrored: byte;    {0 znamena, ze se objekt kresli tak, jak lezi
                         a bezi, cokoli jineho znaci horizontalni prehozeni
                         sprajtu}
{!!!}                   {u objektu typu Text je tady ulozena hodnota
     ...
     NewSprite : byte;  {cislo noveho sprajtu, ktery se ma namalovat
                         na ActivePage }
     Sprite    : byte;  {cislo aktualniho zobrazeneho sprajtu na VisiblePage}
     OldSprite : byte;  {cislo stareho sprajtu na ActivePage }

     ZoomedWidth: integer; {zoomovaci faktor pro sirku sprajtu}
     ZoomedHeigth:integer; {zoomovaci faktor pro vysku sprajtu}
...
{Urceni oblasti, ktere se maji znovuvykreslovat:}
     ...
     SpritesAddressTab: array [0..255] of pointer; {tabulka adres zacatku
                                                  jednotlivych sprajtu }
     {V teto tabulce je vlastne ulozeno, jak za sebou pujdou jednotlive
      sprajty, pokud maji dva sprajty stejny obrazek, staci dat jenom
      dvakrat tu samou sdresu (logicke) }
  end;

var
  AObjsAddressTab: array[0..255] of PAObjTab; {!!!!Bob}
  {TADY kvuli spocitani sirky textu, at se nedela dvakrat; prectu to odsud}

procedure SetTextDimensions(ObjNum: byte; _Text: PString; _Font: PFont);
{!!!Bob}
{Bob}procedure CleanAObjsOrder;
{!bob}var AtJedeINeviditelna: boolean;
{pavel: zrejme switch, ze v mistnosti neni videt myska!?}
...
var
  ...
  AnimMouseSwitchedOn: boolean; {jestli je mys zapnuta}
{  AnimMouseEnabledInt: boolean;}
const
...
procedure CompareMouseWithObjects; forward;
...
{    Vysoko;}
  ...
  if (not AnimMouseSwitchedOn and not AtJedeINeviditelna)or
     (not AnimMouseEnabledInt)then exit;
  {vypadneme, pokud je myska vypnuta a ---je viditelna????---}
{pavel: Roberte, kurva, k cemu je ten zasranej switch?}
  ...
  if AnimMouseSwitchedOn then begin
    {todle je tady kuliva tomu, ze kd}
    {todle je tady zbytecne, protoze semka lezem jedine kdyz je
     myska SwitchedON!}
...
begin
  Inc(LastAObj);
  ObjNum:= LastAObj;
  GetMem(AObjsAddressTab[LastAObj], LengthOf_TAObjTab+(DefLastSprite+1)*4);
  AObjsPriorityTab[LastAObj]:= {DefPriority}0;
  AObjsOrderTab[LastAObj]:= LastAObj;
  with AObjsAddressTab[LastAObj]^ do begin
    TypeOfObj:= DefTypeOfObj;
    Priority:= DefPriority;
    LastSprite:= DefLastSprite;
    DisableErasing:= DefDisableErasing;
...
begin
AddAObj(ObjNum, Image, DefPriority, DefLastSprite, DefDisableErasing,
   StartX, StartY);
end;
...
var
  ...
  TextWidth: byte;   {Nejvetsi sirka textu}
begin
  with AObjsAddressTab[ObjNum]^ do begin
    ...
    repeat begin
      ...
      if TextWidth < WidthOfTextPart(_Font, _Text^, TextFrom, PosInString-TextFrom, False)
      then TextWidth:= WidthOfTextPart(_Font, _Text^, TextFrom, PosInString-TextFrom, False);
      {Ze vsech radku beru sirku toho nejsirsiho jako sirku celeho textu}
      TextFrom:= PosInString+1;
      Inc(LastSprite);         {zvetsil jsem pocet radku}
    end until PosInString= Length(_Text^);
    {v  LastSprite  mame nyni pocet radek-1}
    NewWidth:= TextWidth;
    {V NewWidth je u textu vzdy sirka textu}
    NewHeigth:= (LastSprite+1)*HeigthOfFont(_Font);
...
begin
  Inc(LastAObj);
  ObjNum:= LastAObj;
  GetMem(AObjsAddressTab[LastAObj], LengthOf_TAObjTab+8);
                                    {8=2*4 bajty= 2 adresy: fontu a textu}
  AObjsPriorityTab[LastAObj]:= {DefPriority}0;
  AObjsOrderTab[LastAObj]:= LastAObj;
  with AObjsAddressTab[LastAObj]^ do begin
    TypeOfObj:= Text;
    ...
    (AObjsAddressTab[LastAObj]^.SpritesAddressTab[0]):= _Text;
    (AObjsAddressTab[LastAObj]^.SpritesAddressTab[1]):= _Font;
  end;
  SetTextDimensions(LastAObj, _Text, _Font);
end;
...
begin
  with AObjsAddressTab[ObjNum]^ do begin
    SpritesAddressTab[SpriteNum]:= SpriteAddress;
  end;
end;

procedure AddSpriteToLastObj(SpriteNum: byte; SpriteAddress: pointer);
{zapise adresu urceneho sprajtu do tabulky posledniho objektu }
begin
  with AObjsAddressTab[LastAObj]^ do begin
    SpritesAddressTab[SpriteNum]:= Addr(PWordArray(SpriteAddress)^[0]);
  end;
end;
...
{uvolni posledni animacni objekt (ten, co je nejvic nahore) z tabulky-
 ciste jenom data objektu! Zadne sprajty to neuvolnuje! (Hura!)}
begin
  FreeMem(AObjsAddressTab[LastAObj], LengthOf_TAObjTab+(AObjsAddressTab[LastAObj]^.LastSprite+1)*4);
  Dec(LastAObj);
end;
...
cleanaobjsorder;
  AObjsOrderTab[ObjNum]:= ObjNum; {!!!Bob nepoužívám; pro jistotu spraveno}
  with AObjsAddressTab[ObjNum]^ do begin
    AnimCommand:= AComNone;
    AComCounter:=0;
  end;
...
begin
  with AObjsAddressTab[ObjNum]^ do begin
    ...
    if TypeOfObj= Image then begin
      if (ZoomedWidth<>0)and(ZoomedWidth<>PWordArray(SpritesAddressTab[NewSprite])^[0])
      then NewWidth:= ZoomedWidth
      else NewWidth:= PWordArray(SpritesAddressTab[NewSprite])^[0];
      if (ZoomedHeigth<>0)and(ZoomedHeigth<>PWordArray(SpritesAddressTab[NewSprite])^[1])
      then NewHeigth:= ZoomedHeigth
      else NewHeigth:=  PWordArray(SpritesAddressTab[NewSprite])^[1];
    end else SetTextDimensions(ObjNum, SpritesAddressTab[0], SpritesAddressTab[1]);
  end;
end;
...
{zmeni prioritu objektu}
begin
  AObjsPriorityTab[ObjNum]:= DefPriority;
  AObjsAddressTab[ObjNum]^.Priority:= DefPriority;
end;
...
{nastavi novou barvu pisma u objektu typu Text, pokud neni objekt typu
 Text, posere se to}
begin
  AObjsAddressTab[ObjNum]^.Mirrored:= Color;
end;
...
begin
  with AObjsAddressTab[ObjNum]^ do begin
     ZoomedWidth:= DefZoomedWidth;
     ZoomedHeigth:= DefZoomedHeigth;
    if (ZoomedWidth<>0)and(ZoomedWidth<>PWordArray(SpritesAddressTab[NewSprite])^[0])
    then NewWidth:= ZoomedWidth
    else NewWidth:= PWordArray(SpritesAddressTab[NewSprite])^[0];
    if (ZoomedHeigth<>0)and(ZoomedHeigth<>PWordArray(SpritesAddressTab[NewSprite])^[1])
...
var
  ...
  HelpOrder: byte;
begin
{no bodejť! při uvolnění se decne lastaobj a ztratí se jeho priorita
 a vůbec se to celé zkurví}
cleanaobjsorder;
{for sortedobj:=0 to lastaobj do
  aobjsordertab[sortedobj]:=sortedobj;
{je to opravdu dobře? co neviditelné objekty apod.? ale takto to aspoň
 funguje!}
{!!!sorry, v případě, že se stav anim. obj. nemění, nevadí to...
!ne, blbne při deinstalaci na konci místnosti a skoku do druhé!}
...
{  SortAObjsOrder;}
  ...
  for OrderCompObj:= 0 to LastAObj do begin
    ...
    if ComparedObj=0 then continue; {!!!Bob}
    with AObjsAddressTab[ComparedObj]^ do begin
      if (AObjsPriorityTab[ComparedObj]> 0)
      and(TypeOfObj= AnimWhatObjectsMousePoints)then
        ...
        if(WhereMouseX>=NewX)and(WhereMouseX<NewX+NewWidth)and(WhereMouseY>=NewY)and(WhereMouseY<NewY+NewHeigth)then
          ...
          else if(PByteArray(SpritesAddressTab[NewSprite])^[4+(WhereMouseY-NewY)+
                  ( (WhereMouseX-NewX)*PWordArray(SpritesAddressTab[NewSprite])^[1] ) ]<>255)
          {Kdezto Image...}
  ...
  if OldAnimObjectUnderMouse<>AnimObjectUnderMouse then begin
    OldAnimObjectUnderMouse:=AnimObjectUnderMouse;
    UnderMouseUserProc;
  end;
end;
...
begin
{!!!bob, možná je to proto!!!!!}{cleanaobjsorder;}{ne, to udělá i tato
 procedura:}
  {Setridim vsechny objekty podle vzestupne priority:}
  SortAObjsOrder;

  if AnimEnableClearScreen=0 then begin {pokud neni nastavene mazani}
...
{!!!!!objekt zmizel a zde se to nezastavilo!!!!! takze je to nekde jinde
!!!!!}
...
{bacha na to, tady jsem mel blbou logiku! -- }
                                         { I }
                                         { v }

(*            if (OldX<NewX)or(AnimCommand> AComRepaint)then
            AreaAboveX:= OldX
            else AreaAboveX:= NewX;
            if (OldY<NewY)or(AnimCommand> AComRepaint)then
            AreaAboveY:= OldY
            else AreaAboveY:= NewY;
*)
...
{sortaobjsorder;}
if false and((aobjsprioritytab[61]<=0)or(aobjsaddresstab[61]^.animcommand=acomunvisible)) then begin
{!existuje ještě další důvod k nevykreslení --- po průchodu jeho POCHYBNÝM
 cyklem areax apod... vyjde pochybně...
 už chápu tvůj cyklus --- jak dělá ty obj. s menší a větší prioritou; nevím,
 zda-li by to šlo líp, ale Ty to děláš úplně strašně (pro každý přečnívající
 objekt putneš právě tu oblast, takže něco putneš hrozně mockrát!!!!!)}
closegraph;
writeln('###');
halt(1);
end;
          ...
          if(AnimCommand<>AComUnvisible)then begin
            if TypeOfObj= Image then begin {pokud se jedna o obrazek, budeme}
               ...
               if(AreaAboveWidth<>0)then begin
                 ...
                 if (NewWidth=PWordArray(SpritesAddressTab[NewSprite])^[0])
                 and (NewHeigth=PWordArray(SpritesAddressTab[NewSprite])^[1])
                 then begin
                   {tisknu nezoomovane}
                   if Mirrored=0 then begin
                     PutMaskImagePart(NewX, NewY, 0, 0, 320, 200, SpritesAddressTab[NewSprite]);
                   end else begin
                     PutMirrorMaskImagePart(NewX, NewY, 0, 0, 320, 200, SpritesAddressTab[NewSprite]);
                   end;{if Mirrored=..}
                 end else begin
                   {tisknu zoomovane}
                   if Mirrored=0 then begin
                     PutMaskImagePartZoom(NewX, NewY, NewWidth, NewHeigth,
                      0, 0, 320, 200, SpritesAddressTab[NewSprite]);
                   end else begin
                     PutMirrorMaskImagePartZoom(NewX, NewY, NewWidth, NewHeigth, 0, 0, 320, 200, SpritesAddressTab[NewSprite]);
                   end;{if Mirrored=..}
                 end;
               end else begin
                 ...
                 if OrderPaintObj>0 then for OrderCompObj:=0 to OrderPaintObj-1 do begin
                   ...
                   if (AObjsPriorityTab[ComparedObj]>0)and(AreaWidth<>0) then begin
                     ...
                     then begin
                       {tisknu nezoomovane}
                       if Mirrored=0 then begin
                         PutMaskImagePart(NewX, NewY, AreaX, AreaY, AreaWidth, AreaHeigth, SpritesAddressTab[NewSprite]);
                       end else begin
                         PutMirrorMaskImagePart(NewX, NewY, AreaX, AreaY, AreaWidth, AreaHeigth, SpritesAddressTab[NewSprite]);
                       end;{if Mirrored=..}
                     end else begin
                       {tisknu zoomovane}
                       if Mirrored=0 then begin
                         PutMaskImagePartZoom(NewX, NewY, NewWidth, NewHeigth,
                           AreaX, AreaY, AreaWidth, AreaHeigth, SpritesAddressTab[NewSprite]);
                       end else begin
              ...
              TextFrom:= 1;
              TextTo:= 0;
              for TextLine:= 0 to LastSprite do begin
                TextTo:= PosFrom(PString(SpritesAddressTab[0])^, '|', TextTo+1);
                PrintedText:= Copy(PString(SpritesAddressTab[0])^, TextFrom, TextTo-TextFrom);
                PrintText(NewX+( (NewWidth-WidthOfText(SpritesAddressTab[1], PrintedText)) div 2),
                  NewY+TextLine*HeigthOfFont(SpritesAddressTab[1]) , PrintedText, SpritesAddressTab[1]);
                TextFrom:= TextTo+1;
              end;
            end;{if (TypeOfObject...}
          end;
        ...
        with AObjsAddressTab[ActualObj]^ do begin
          if TypeOfObj= Image then begin
            ...
            then begin
              ...
              {tisknu zoomovane}
              if Mirrored=0 then begin
                PutMaskImagePartZoom(NewX, NewY, NewWidth, NewHeigth, 0, 0, 320, 200, SpritesAddressTab[NewSprite]);
              end else begin
                PutMirrorMaskImagePartZoom(NewX, NewY, NewWidth, NewHeigth, 0, 0, 320, 200, SpritesAddressTab[NewSprite]);
              end;{if Mirrored=..}
            end;
