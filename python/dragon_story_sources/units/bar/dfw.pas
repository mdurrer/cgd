...
hroutí se při ladění tohoto i BP při ^F4 a MemAvail!!!!!
 ...
 bob 22.11.94: vylepsena cloaditem, at se pri chybe cteni nepokousi zavirat
               file_path
 kroco : how muzes napsat kvalitni (je to sileny ksindl), rychly (tak
         debilne pomaly) a s velkou kapacitou (do 64kb) archiv?????
         + s omezenim na max. number souboru v archivu + nemuzes soubory
...
{**                        DFW  -  DATA FILE WORK                          **}
{**                   chybny code :    by PETR                              **}
{**               spravny code(moc ho neni) : vsichni ostatni               **}
...
{****************************************************************************}
{$R-,S-,V-,I-,A+}
{U vsech vysledku logickych funkci: False znamena didnt_succeed !}
...
Function FileExist(FName : String) : Boolean; {Zjisti jestli file_path existuje}
{ARCHIVACE BEZ compression}
...
Function LoadItem(Archiv : String; Var Destination : Pointer; Which : Word) : Word; {Nahraje file_path do pameti}
...
Var F,F1:File;                   {Soubory}
    SizeTable:Word;              {length_ itemu v archivu}
    PostTable:LongInt;           {position itemu v archivu}
    Buffer:MyPointer;            {Pro komprimaci}
    ...
    RError, RErrors:Boolean;     {I/O on_error pri zapisu/cteni archivu}
    Files:Word;                  {number itemu v archivu}
    Kapacita,StKapacita:Word;    {Maximalni kapacita archivu}
    CSize:Word;                  {length_ souboru v archivu}
    SuppSize:Word;               {length_ dodavaneho itemu}
...
{Vysledkem je length_ zkomprimovaneho pointeru Source}
Function CompSize(Source:MyPointer):Word;
begin
...
{path,Pointer,Velikost nekomprimovaneho(Kvuli auto. mem. rezervaci)}
Function SaveData(FName: String; Source : Pointer; Size : Word) : Boolean;
Var BSize:Word;
begin
...
begin
  ...
  If IOResult<>0 then RError:=False;
  Close(F); {Uzavreni souboru}
  DeCompress(Buffer,Target); {decompression do zvoloneho pointeru}
...
begin
  ...
  If IOResult<>0 then RError:=False; {Pokud on_error}
  Close(F); {Uzavre file_path}
  {Pokud byla on_error, vysledek funkce je 0}
...
{Zjistuje, jestli je file_path na disku}
Function  FileExist(FName : String) : Boolean;
Var ExF:File;
begin
  Assign(ExF,FName);
  Reset(ExF,1); {Otevre file_path}
...
Procedure MakeHead;
begin
  SizeTable:=0; {Vynuluje tyto variables,}
...
{Pridava file_path Supplement do archivu, pri uspechu je vysledkem number_ souboru
v archivu}
...
begin
  ...
  Reset(F1,1); {Otevre dodavany file_path}
  If IOResult<>0 then begin AddFromFile:=0; Exit; end;
  {Pokud je length_ souboru vetsi nez 1 pointer, archivace se neprovede}
  ...
  {Pokud je file_path delsi, nez je mozne, vynada ti to}
  If Not FileExist(Archiv) then {Pokud file_path neexistuje}
  begin
    ...
    If IOResult<>0 then RError:=False;
    MakeHead;
    PostTable:=FilePos(F); {Zapsani position prvniho s. do tabulky}
  ...
  begin
    ...
    SizeTable:=SuppSize;
    SaveHead(Files);
    Seek(F,PostTable); {Nastaveni na end_ souboru = position noveho souboru}
  end;
...
begin
  ...
  begin
    Reset(F,1);
    BlockRead(F,Files,2); {Nacteni poctu souboru v archivu}
    BlockRead(F,Kapacita,2); {Podiva se kolik je maximalne mozno ulozit file_.}
    If IOResult<>0 then RError:=False;
    If Files=Kapacita then
    begin
...
{Nazev archivu, pointer, number_ souboru (1-255)}
...
{Vysledkem je length_ souboru Which v archivu pri neuspechu 0}
...
{Zkomprimuje externi file_path Supplement do archivu, vysledek number_ souboru}
Function CAddFromFile(Archiv : String; Supplement : String) : Word;
Var Dest:Pointer;
begin
  RError:=True;
  ...
  begin
    ...
    Rewrite(F,1);
    BlockWrite(F,Files,2); {Zapsani poctu souboru do archivu}
    BlockWrite(F,StKapacita,2); {Zapise max number souboru}
    If IOResult<>0 then RError:=False;
    BlockWrite(F,FileHead,2); {Zapsani napisu LIST:}
    If IOResult<>0 then RError:=False;
  ...
  begin
    ...
    BlockWrite(F,Files,2); {Zapsani poctu souboru v archivu}
    If IOResult<>0 then RError:=False;
    PostTable:=FileSize(F); {length_ F=position na konci souboru}
...
{pridano navic 200 byte}
  ...
  begin
    ...
    If IOResult<>0 then RError:=False;
    MakeHead;
    {Vytvori zatim prazdnou tabulku pro number souboru StKapacita}
    PostTable:=FilePos(F);
    {Zapamatuju si pozici, where_to prijde "telo" itemu s jeho daty}
    SizeTable:=CSize;
    {SizeTAble je length_ komprimovanych dat (length_ itemu v archivu)}
    SaveHead(Files);
    {Zapise do tabulky delku compr. itemu a jeho pozici od zacatku archivu}
    Seek(F,PostTable);
    {Nastavim se na pozici, where_to prijdou data}
  end else
  begin
    Reset(F,1);
    BlockRead(F,Files,2); {Nacteni poctu souboru v archivu}
    ...
    Seek(F,0); {Nastaveni na indikator poctu souboru = zacatek archivu}
    BlockWrite(F,Files,2);
    {Aktualizuji number souboru v archivu, zapisuji novou hodnotu}
    If IOResult<>0 then RError:=False;
    PostTable:=FileSize(F);
    {v PostTable je position dat; prijdou na end_ archivu, takze vezmu rovnou}
    {jeho delku}
    SizeTable:=CSize;
    {SizeTable je length_ komprimovanych dat (length_ itemu v archivu)}
    SaveHead(Files);
...
{Dekomprimuje file_path Which z archivu do pointeru Destination}
Function CLoadItem(Archiv :  String; Var Destination : Pointer; Which : Word) : Word;
Var UnCSize : Word; {Budouci length_ dekomprim. souboru << 65536}
label on_error;
begin
  RError:=True;
  Assign(F,Archiv);
  Reset(F,1);
  If IOResult<>0 then begin RError:=False; goto on_error end;
  LoadHead(Which);
  Seek(F,PostTable); {Nastavi se na pozici souboru v archivu}
  BlockRead(F,UnCSize,2); {Nacte delku rozpakovaneho souboru, kvuli pameti}
  GetMem(Destination,UnCSize); {Rezervace pameti pro rozpakovany file_path}
...
{Pri uspechu je vysledkem length_ ROZBALENEHO souboru v komprimovanem archivu}
...
Function PackedItemSize(Archiv : String; Which : Word) : Word; {length_ komprimovaneho souboru}
begin
  RError:=True;
  Assign(F,Archiv);
  Reset(F,1);
...
Var FLength,RDB:LongInt; {length_ archivu v originale}
begin
  ...
  Repeat
    ...
    begin
      Inc(CRC,Buffer^[Ld]); {Pricte hodnotu bytes k CRC}
    end;
...
Var RLength,FLength,RCRC:LongInt; {length_ archivu v originale}
begin
  RError:=True;
  TestArchiveCRC:=True;
  CRC:=0;
...
begin
  ...
  begin {Tady je mazani Itemu, ktery ma za sebou jeste dalsi Item(y)}
    NewPosition:=PostTable; {position smazaneho itemu}
...
begin
  ...
  If IOResult<>0 then RError:=False;
  BlockRead(F,Files,2);
  {Je nastaveny na zacatek archivu, precte number obsazenych mist v archivu}
  If IOResult<>0 then RError:=False;
  LoadHead(Which);
...
begin
  ...
  Reset(F,1);
  If IOResult<>0 then RError:=False;
  BlockRead(F,Files,2);                 {Nacte number souboru}
  BlockRead(F,Kapacita,2);              {Nacte aktualni kapacitu}
  If IOResult<>0 then RError:=False;
  Souboru:=Files;                       {Uschova number pro vlast. potrebu}
  ...
  If IOResult<>0 then RError:=False;
  Seek(F,(Files+1)*6);                  {Prvni position za posledni}
...
begin
  ...
  If IOResult<>0 then RError:=False;
  Souboru:=Files;                       {Zalohuje si number souboru}
  If KMuch+Kapacita>65535 then
  begin Close(F); Exit; CExpandArchive:=True; end; {end_}
  Repeat
  ...
  begin
    BlockWrite(F,FileHead,2);           {...number mist v tabulce}
    BlockWrite(F,NewPosition,4);
  end;
  If IOResult<>0 then RError:=False;
  Close(F);                             {Uzavre file_path}
...
Vznyklo to tym, how jsem unyt pomocy replace predelaval z TRUE NA FALSE a
